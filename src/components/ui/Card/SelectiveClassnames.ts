export const dynamicClassesSelective = (hasTitle: boolean, className: string) => {
  const mainClasesParentDiv = hasTitle
    ...
    : `relative xl:flex flex md:flex sm:flex-row  items-center w-full md:w-[300px] lg:w-[437px] xl:w-[537px] px-[16px] rounded-l-lg  select-none py-[0px] rounded-r-xl bg-white ${className}`;
  ...
  const iconContainerStyle = hasTitle
    ...
    : 'h-[4.25rem] max-w-[4rem] max-h-[4.25rem] mim-w-[4rem]  w-full flex items-center rounded-[0.75rem] bg-white group-hover:bg-primario-20';

const dynamicClassesSelective = (hasTitle: boolean, className: string, activeCard: boolean) => {
  ...
  const mainClasesParentDiv = hasTitle
    ...
    : `relative xl:flex flex md:flex sm:flex-row  items-center  px-[16px] rounded-l-lg  select-none py-[0px] rounded-r-xl ${className} cursor-pointer`;
  ...
  const iconContainerStyle = hasTitle
    ...
    : 'h-[4.25rem] max-w-[4rem] max-h-[4.25rem] mim-w-[4rem]  w-full flex items-center rounded-[0.75rem]  group-hover:bg-primario-20 cursor-pointer';
